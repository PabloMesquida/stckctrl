...
const getProduct = async (req, res) => {
  ...
  try {
    ...
    const result_info_prod_colors = await executeQuery({
      query:
        "SELECT p.id_color, c.color, c.hex FROM p_colores AS p JOIN colores AS c ON p.id_color = c.id WHERE id_prod = ? ",
      values: [id],
    });
    ...
    const result_info_prod_stock = await executeQuery({
      query:
        "SELECT pt.id, pt.id_prod_color, pt.id_talle, pt.stock, c.color, t.talle, c.id, c.hex AS id_color FROM p_talles AS pt JOIN talles AS t ON t.id = pt.id_talle JOIN p_colores AS pc ON pc.id = pt.id_prod_color JOIN colores AS c ON c.id = pc.id_color WHERE pt.id_prod = ?",
      values: [id],
    });
